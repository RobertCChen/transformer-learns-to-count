...
def main(argv=None):
    ...
    dataset = SubjectVerbAgreement

    # acc_list = []
    # max_acc = None
    # for stack_size in range(1, 21):
    #     # x, y, m = dataset.get_seq(stack_size, 1, 2, 0, 128)
    #     x, y, m = dataset.get_seq(stack_size)
    ...
    stack_size = 8 # Change this value to test longer / shorter sequences
    # x, y, m = dataset.get_seq(stack_size, 1, 2, 0, 2 * stack_size + 1)
    x, y, m = dataset.get_seq(stack_size, plurality_array=[0, 0, 1, 1, 0, 0, 1, 1])
    model.eval()
    yhat = model(x.unsqueeze(1))
    hdn = model.hidden_state # batch x seq x hdn
    loss, acc = loss_fn(y.unsqueeze(1), yhat, m.unsqueeze(1))
